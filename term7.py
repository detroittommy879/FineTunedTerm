...
class TerminalWidget(QTextEdit):
    ...
    terminalFocusGained = pyqtSignal(QWidget)

    ansi_escape_regex = re.compile(
        r'(?:'
        r'\x1b\[([\d;?]*)m'               # SGR
        r'|\x1b\]([012]);([^\x07]*)(?:\x07|\\)'  # OSC
        r'|\x1b\[([\d;]*)([A-Za-z])'      # CSI
        r'|\x1b([()][012AB])'             # Char set
        r'|\x1b([ENOM])'                  # Other modes
        r'|\x1b[=>])'                     # Terminal modes
    )
    ...
    def __init__(self, parent=None, terminal_id="Unknown", initial_font=None):
        ...
        # Font & style
        self.current_font   = initial_font or QFont("Courier New", 10)
        ...
        # QProcess (we'll sometimes wrap it in winpty)
        ...
        # Start after Qt event loop spins
        QTimer.singleShot(0, self.start_process)

    def apply_font(self, font):
        """Apply a new font to this terminal"""
    ...
    def _apply_sgr_code(self, code):
        """Apply SGR (Select Graphic Rendition) code to format"""
        if not code:
            self._reset_char_format_to_default()
            return
        ...
        for part in code.split(';'):
            ...
            elif 30 <= num <= 37 or 90 <= num <= 97:  # Foreground color
                self.current_format.setForeground(ANSI_SGR_CODES_TO_COLORS.get(num, DEFAULT_FG_COLOR))
            elif 40 <= num <= 47 or 100 <= num <= 107:  # Background color
                self.current_format.setBackground(ANSI_SGR_CODES_TO_COLORS.get(num, DEFAULT_BG_COLOR))
            elif num == 39:  # Default foreground color
                self.current_format.setForeground(DEFAULT_FG_COLOR)
            elif num == 49:  # Default background color
                self.current_format.setBackground(DEFAULT_BG_COLOR)

    def append_ansi_text(self, text: str):
        """Parse ANSI escape sequences and append text with proper formatting"""
        cursor = self.textCursor()
        cursor.movePosition(QTextCursor.MoveOperation.End)
        
        # Process text with ANSI codes
        last_end = 0
        for match in self.ansi_escape_regex.finditer(text):
            # Add any text before this escape sequence with current format
            if match.start() > last_end:
                normal_text = text[last_end:match.start()]
                cursor.insertText(normal_text, self.current_format)
            
            # Process the escape sequence
            sgr_code = match.group(1)
            if sgr_code is not None:
                self._apply_sgr_code(sgr_code)
            elif match.group(2) == '0':  # OSC Set window title
                title = match.group(3)
                self.titleChanged.emit(title)
            # Could handle other escape sequences here
    ...
    def handle_process_error(self, err):
    ...
    def start_process(self):
        """Start a shell.  On Windows+Git‐bash we wrap in winpty if available."""
        ...
        if 0 < (now - self.last_started) < 2:
            ...
            if self.restart_count > 3:
                self.append_ansi_text(f"\x1b[31m⚠️ Too many restarts. Shell disabled.\x1b[0m\n")
                return
        else:
            self.restart_count = 0
        ...
        # Find bash or fallback
        shell_exe = (shutil.which("bash") or
                     shutil.which("powershell.exe") or
                     shutil.which("cmd.exe"))
        if not shell_exe:
            self.append_ansi_text(f"\x1b[31m⚠️ No shell found.\x1b[0m\n")
            return

        # Default args for an interactive login shell
        args = []
        if "bash" in os.path.basename(shell_exe).lower():
            args = ["--login", "-i"]

        # ——— Windows+Git-bash: wrap in winpty if we can find it ———
        program = shell_exe
        if sys.platform.startswith("win") and "bash" in shell_exe.lower():
            winpty = shutil.which("winpty") or shutil.which("winpty.exe")
            if winpty:
                program = winpty
                # winpty [shell_exe ... ]
                args = [shell_exe] + args
                logger.debug(f"[{self.terminal_id}] Wrapping bash in winpty: {program} {' '.join(args)}")
            else:
                logger.warning(f"[{self.terminal_id}] winpty.exe not found – continuing without TTY wrapper.")

        logger.info(f"[{self.terminal_id}] Starting shell: {program} {' '.join(args)}")
        self.append_ansi_text(f"[{self.terminal_id}] Starting {os.path.basename(program)} {' '.join(args)}...\n")

        self.process.start(program, args)
        if not self.process.waitForStarted(3000):
            logger.error(f"[{self.terminal_id}] Failed to start shell: {program}")
            self.append_ansi_text(f"\x1b[31m⚠️ Failed to start shell: {os.path.basename(program)}.\x1b[0m\n")
            self.process_running = False
        else:
    ...
    def process_finished(self, exitCode, exitStatus):
        status = "crashed" if exitStatus == QProcess.ExitStatus.CrashExit else "finished"
        msg = f"\n\x1b[33m[{self.terminal_id}] Shell {status} (code {exitCode}).\x1b[0m\n"
        logger.info(f"[{self.terminal_id}] Process finished: {exitCode}, {exitStatus}")
        self.append_ansi_text(msg)
        self.process_running = False

    def keyPressEvent(self, ev):
        """Send all keystrokes directly into the pty (winpty → stdin)."""
        ctrl = ev.modifiers() & Qt.KeyboardModifier.ControlModifier
        # Ctrl+C → SIGINT
        if ev.key() == Qt.Key.Key_C and ctrl:
        ...
        seq = arrow_map.get(ev.key())
        if seq:
            self.process.write(seq)
            return
