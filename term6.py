...
class TerminalWidget(QTextEdit):
    ...
    ansi_escape_regex = re.compile(
        r'(?:'
         r'\[([\d;?]*)m'  # SGR
        r'|\]([012]);([^]*)(?:|\\)'  # OSC
        r'|\[([\d;]*)([A-Za-z])'  # CSI (Control Sequence Introducer)
    ...
    def append_ansi_text(self, text: str):
        ...
        for m in self.ansi_escape_regex.finditer(text):
            ...
            sgr, osc_t, osc_c, csi_p, csi_l, cs = m.groups()
            ...
            # Enhanced CSI handling for cursor and erase operations
            elif csi_p is not None and csi_l is not None:
                # Parse CSI parameters
                params = []
                if csi_p == "" or csi_p is None:
                    params = [0]
                else:
                    for part in csi_p.split(';'):
                        if part == '':
                            params.append(0)
                        else:
                            try:
                                ...
                                params.append(0)
                first_param = params[0] if params else 0
                ...
                if csi_l == 'J':
                    ...
                    if mode == 0:
                        ...
                        # Erase entire screen and reset format
                        self.clear()
                        self._reset_char_format_to_default()
                    ...
                    for _ in range(max(row - 1, 0)):
                        cursor.movePosition(QTextCursor.MoveOperation.NextBlock)
                    cursor.movePosition(QTextCursor.MoveOperation.StartOfBlock)
                    for _ in range(max(col - 1, 0)):
                        cursor.movePosition(QTextCursor.MoveOperation.Right)
                    self.setTextCursor(cursor)
                elif csi_l == 'A':
                    count = first_param if first_param > 0 else 1
